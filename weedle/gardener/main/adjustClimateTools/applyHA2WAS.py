...
def applyHA2WAS(direction,adjustment):
    try:
        if direction == 'DECREASE':

            print "------------------------"
            print "= IN DECREASE AIR HUMIDITY"
            print "------------------------"
            water = config.readweedleAdjustment("HAwater")
            air = config.readweedleAdjustment("HAair")
            sun = config.readweedleAdjustment("HAsun")
            print "============================"
...
### LOG THE ACTION
        ...
        with open(gardenerActions, "a") as f:
            f.write('HA2WAS|'+str(datetime.datetime.now())+"|"
